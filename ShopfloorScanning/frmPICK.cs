...
namespace ShopfloorScanning
{
    public partial class frmPICK : Form
    {
        ...
        private bool bIsRubber;
        private string sRubberLotNumber;
        ...
        private void CleanData()
        {
            ...
            bIsRubber = false;
            sRubberLotNumber = "";
            txtRubberLotNumber.Text = "";
            txtRubberLotNumber.Enabled = false;
            ClearStockList();
        }
        ...
        private void txtSequenceNumber_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (bIsRubber) txtRubberLotNumber.Focus(); else grStock.Focus();
            }
        }
            ...
            else
            {
                ...
                if (sErrorMsg == "")
                {
                    ...
                    if (mWorkcenterCheckDataTable.Rows.Count > 0)
                    {
                        ...
                        lblQtyRequiredData.Text = mWorkcenterCheckRow.RequiredQuantity.ToString().Trim();
                        lblQtyIssuedData.Text = mWorkcenterCheckRow.IssuedQuantity.ToString().Trim();
                        ...
                        lblQtyRemainingData.Text = dRemQty.ToString().Trim();
                    ...
                    //check if rubber
                    if (!String.IsNullOrEmpty(mQueriesTableAdapter.CheckIfRubber(iComponentItemKey).ToString()))
                    {
                        bIsRubber = true;
                        txtRubberLotNumber.Enabled = true;
                        txtRubberLotNumber.Focus();
                    }
                }
            }
        ...
        private void grStock_CurrentCellChanged(object sender, EventArgs e)
        {
            if (grStock.CurrentCell != null)
            {
                ...
                sRubberLotNumber = grStock.Rows[0].Cells["StockLotNumber"].Value.ToString();

                if (bIsRubber)
                {
                    txtRubberLotNumber.Text = sLotNumber;
                    txtRubberLotNumber.Select(0, 50);
                    txtRubberLotNumber.Focus();
                }
            }
        }
        ...
        private void grStock_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (bIsRubber) txtRubberLotNumber.Focus(); else txtQuantity.Focus();
            }
        }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (fstCon.fstiClient.ProcessCDF(sLine, sFSUserID))
            {
                //all OK
                if (sComponentType == "R")
                {
        ...
        private void txtRubberLotNumber_Validating(object sender, CancelEventArgs e)


        {
            if (bIsRubber && (txtRubberLotNumber.Text != sRubberLotNumber))
            {
                MessageBox.Show(this, "Please scan first Rubber Lot Number from the stock list according to Expiration Date. First Lot No. " + sRubberLotNumber, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);

                txtRubberLotNumber.Text = sRubberLotNumber;
                txtRubberLotNumber.Select(0, 50);
                GetStockList();
                e.Cancel = true;
            }
        ...
        private void txtRubberLotNumber_KeyDown(object sender, KeyEventArgs e)
        {
